...
init -100:
    ...
    transform spriteanimation(animation_name):
        function sprite_animation(animation_name)
...
init -50 python:
    ...
    class SpriteAnimation:
        ...
        def __init__(self, anim_list, loop_time=None):
            if anim_list is None or len(anim_list) < 1:
                return
            self.anim_list = anim_list
            self.loop_time = loop_time

        def __call__(self, st, at):
            if self.anim_list is None or len(self.anim_list) < 1:
                return None

            current_time = int(round(st * 1000))
            if self.loop_time is not None:
                current_time = current_time % self.loop_time

            anim_index = None
            for i in range(len(self.anim_list)):
                frame_time, _ = self.anim_list[i]
                if current_time >= frame_time:
                    anim_index = i
                else:
                    break
            if anim_index is None:
                return self.anim_list[0][1], None

            row = self.anim_list[anim_index]
            next_row = None
            if anim_index+1 < len(self.anim_list):
                next_row = self.anim_list[anim_index+1]
            elif self.loop_time is not None and len(self.anim_list) > 1:
                next_row = self.anim_list[0]

            frame_time, image_name = row
            if next_row is None:
                return image_name, None
            nextframe_time, nextimage_name = next_row
            if nextframe_time >= current_time:
                return image_name, (nextframe_time - current_time) / 1000.0
            else:
                return image_name, (self.loop_time - current_time) / 1000.0
            return None
    ...
    hasmouthlist = []
    layerorder = []
    def DefineImages(imageFolder, composite=False, prepend=None, autoEmotes=False, overrideCharname=None, overrideLayerOrder=None, offsets=None, zooms=None, sides=None, invertNames=0):
        global layerorder
        animlist = []
        if composite:
            imglist = []
        ...
        for path in renpy.list_files():
            if path.startswith(imageFolder + "/"):
                ...
                dir_list = tuple(prepend + dir_list[excludeFirstXFolders:])
                lowerpath_list = tuple([ x.lower() for x in path_list ])

                if path.lower().endswith('.txt'):
                    animlist.append(path_list)
                    pathdict[path_list] = path
                    contents = renpy.file(path).readlines()
                    sprite_anim = []
                    sprite_loop = None
                    for line in contents:
                        ...
                        if ' ' not in line:
                            ...
                            try:
                                t = int(line[0:line.index(' ')])
                                n = line[line.index(' ')+1:]
                                if n == 'repeat':
                                    sprite_loop = t
                                else:
                                    n = '_'.join(dir_list + tuple(n.split(' ')))
                                    sprite_anim.append((t, n))
                            except:
                                pass
                    if len(sprite_anim) > 0:
                        renpy.image(('_'.join(lowerpath_list),), DynamicDisplayable(SpriteAnimation(sprite_anim, sprite_loop)))
                    if composite:
                ...
                if composite:
                    if invertNames > 0:
                        path_list = path_list[invertNames:invertNames+1] + path_list[:invertNames] + path_list[invertNames+1:]
                        lowerpath_list = lowerpath_list[invertNames:invertNames+1] + lowerpath_list[:invertNames] + lowerpath_list[invertNames+1:]
                    renpy.image(('_'.join(path_list),), path)
                    if lowerpath_list != path_list:
                        renpy.image(('_'.join(lowerpath_list),), path)
                else:
                    if invertNames > 0:
                        path_list = path_list[invertNames:] + path_list[:invertNames]
                        lowerpath_list = lowerpath_list[invertNames:] + lowerpath_list[:invertNames]
    ...
    def DynamicSprites_VarUpdate():
        ...
        if len(charlist) == 0 or len(posedict) == 0:
            ...
            valueszoom = originalsizedict[basepath] - 50. / config.screen_height
    ...
    class DynamicSprites_ChangeSelectedLayer:
        ...
        def __call__(self):
            ...
            DynamicSprites_VarUpdate()
    class DynamicSprites_ChangeSelectedEmote:
        def __init__(self, emote):
            self.emote = emote
        def __call__(self):
            global dynamicspritespreview_var_selectedlayers, dynamicspritespreview_var_selectedpose, dynamicspritespreview_var_selectedlayers, layerorder
            for layer in layerorder:
                if layer in self.emote:
                    img_name = self.emote[layer].image.name[0]
                    i = 0
                    for vl in valuespath[layer]:
                        if vl == img_name:
                            dynamicspritespreview_var_selectedlayers[layer] = i
                            break
                        i += 1
            DynamicSprites_VarUpdate()
    ...
    button:
        ...
        hbox:
            ...
            frame:
                ...
                xysize (int(config.screen_width/2), 1.0)
                margin (10,50,5,10)
                padding (5,5)
                frame:
                    background None
            ...
            button:
                background Solid("fffe")
                xysize (int(config.screen_width/4), 1.0)
                margin (5,50,5,10)
                padding (5,5)
                action Hide("dynamicspritespreview_dropdown")
                vbox:
                        ...
                        if k[0] == dynamicspritespreview_text_selectedchar:
                            for e in sorted(mapemotedict[k]['emotesdict'].keys()):
                                button:
                                    ...
                                    action [
                                        Hide("dynamicspritespreview_previewpopup"),
                                        DynamicSprites_ChangeSelectedEmote(mapemotedict[k]['emotesdict'][e]),
                                    ]
                                    text ' '.join(k[1:]+(e,)):
                                        size 18
                                        color "fff"
